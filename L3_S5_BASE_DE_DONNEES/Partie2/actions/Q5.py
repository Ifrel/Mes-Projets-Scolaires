...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.title(
            "Q5 : températures (moyennes, min, max) par département et par année (version optimisée)"
        )
    ...
    def extract_data(self):
        ...
        try:
            # Une seule requête pour obtenir toutes les données nécessaires en
            # une jointure
            query = """
        ...
        texte = f"Le calcul ({len(tab)} lignes) a pris {round(
            end - start, 3)} secondes et exécuté {nbquery} requête(s)."
        self.timelabel.configure(text=texte)
