...
class Window(tk.Toplevel):
    ...
    # Attributs de la classe (pour être en mesure de les utiliser dans les
    ...
    def __init__(self, parent):
        ...
        self.title(
            "Q3 : nombre de mesures prises et moyenne des temperatures [...] (version dynamique)"
        )
        display.defineGridDisplay(self, 3, 3)
        # On donne un poids plus important à la dernière ligne pour l'affichage
        # du tableau
        ...
        list_regions = []
        list_r = (
            db.data.cursor().execute("""SELECT nom_region FROM Regions""").fetchall()
        )
        for r in list_r:
            list_regions.append(r[0])

        # Combobox pour le choix de la région
        self.input = ttk.Combobox(self, values=list_regions, state="readonly")
        self.input.grid(row=1, column=1)
        self.input.bind(
            "<<ComboboxSelected>>", self.searchRegion
    ...
    # TODO Q3 Modifier la fonction searchRegion pour un choix dynamique de la
    # région
    def searchRegion(self, event=None):

        # On vide le treeView (pour rafraichir les données si quelque chose
        # était déjà présent)
        ...
        if len(region) == 0:
            self.errorLabel.config(
                foreground="red", text="Veuillez saisir une région !"
            )
        ...
        # Si la saisie contient quelque chose
            ...
            try:
                ...
                result = cursor.execute(
                    """SELECT code_departement, nom_departement, COUNT(date_mesure), AVG(temperature_moy_mesure)
            ...
            # Si tout s'est bien passé
                ...
                for row in result:
                    ...
                    i += 1
                # On affiche un message à l'utilisateur en fonction du nombre
