...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        # On bind l'appui de la touche entrée sur la case de saisie, on peut
        # donc utiliser soit la touche entrée soit le bouton valider
        self.input.bind("<Return>", self.searchRegion)
        ttk.Button(self, text="Valider", command=self.searchRegion).grid(
            row=0, column=2
        )
    ...
    # Soit via le bind qui a été fait sur la case de saisie quand on appuie
    # sur Entrée, dans ce cas bind fournit un event (que l'on utilise pas ici)
