...
int bitread(BFILE* fichier) {
    ...
    if ((fichier->nombre == 0) && !beof(fichier)) {
        ...
        if (resultat == DELIMITEUR) {
            resultat = fgetc(fichier->fichier);
            if (resultat == EOF) /* Bug, ceci ne devrait pas arriver */
                return -1;
            if (resultat == DELIMITEUR) {
                fichier->nombre = 8;
            } else {
                resultat -= '0';
                if ((resultat < 1) || (resultat > 8)) /* Bug si resultat > 8 */
                    return -1;
                int nombre;
                nombre = resultat;
                resultat = fgetc(fichier->fichier);
                if (resultat == EOF) /* Bug, ne devrait pas arriver */
                    return -1;
...
int bitwrite(BFILE* fichier, int bit) {
    ...
    if (fichier->nombre == 8) {
        ...
        if (fichier->donnees == DELIMITEUR) {
            ...
            if (resultat == EOF) /* Erreur d'ecriture */
                return -1;
        }
        resultat = fputc(fichier->donnees, fichier->fichier);
        if (resultat == EOF) /* Erreur d'ecriture */
