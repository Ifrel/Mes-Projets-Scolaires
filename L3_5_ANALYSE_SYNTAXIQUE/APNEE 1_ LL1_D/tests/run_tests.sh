...
# Script de test coloré et verbeux : compile puis compare assignations
# à un interpréteur Python de référence.
...
def parse_func():
    ...
    if tok.typ == "COS":
        ...
        return math.cos(parse_assign())
    if tok.typ == "SIN":
        consume("SIN")
        # return math.cos(parse_expr())
        return math.sin(parse_assign())
